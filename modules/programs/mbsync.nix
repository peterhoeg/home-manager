...
let
  ...
  hasXdgSupport = lib.versionAtLeast cfg.package.version "1.5.0";
...
in {
  ...
  config = mkIf cfg.enable (mkMerge [
    ...
    {
      home = {
        activation = mkIf (mbsyncAccounts != [ ]) {
          createMaildir =
      ...
      } // lib.optionalAttrs (! hasXdgSupport) {
        file.".mbsyncrc".text = configFile;
      };

      programs.notmuch.new.ignore = [ ".uidvalidity" ".mbsyncstate" ];

      xdg.configFile = lib.optionalAttrs hasXdgSupport {
        "isyncrc".text = configFile;
      };
    }
  ]);
}
